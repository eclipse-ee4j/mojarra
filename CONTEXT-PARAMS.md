...
<tr><td><code>StateSavingMethod</code></td><td><code>SERVER</code></td><td>1.0</td><td>request</td><td>Where the view state is saved. <code>server</code> keeps it in the session and sends only a reference to the client, and is the faster one, but it is bounded by <code>org.glassfish.mojarra.numberOfViewSequencesInSession</code> times <code>org.glassfish.mojarra.numberOfViewsPerViewSequence</code> and is lost when the session is invalidated, both of which surface as a <code>ViewExpiredException</code>. <code>client</code> sends the entire state to the client, which is effectively unbounded and survives session invalidation, at the price of CPU and bandwidth on every request.</td></tr>
</tbody>
</table>
...
<tr><th colspan="5" align="left"><br/><code>org.glassfish.mojarra.enableThreading</code></th></tr>
<tr><td><code>boolean</code></td><td><code>false</code></td><td>1.2_13</td><td>startup</td><td>Gates the background thread which watches the Faces configuration files for changes and reloads the application, but only when the project stage is <code>Development</code>, where that watch is already active unless this parameter is explicitly set to <code>false</code>. It no longer has any effect on how those files are parsed.</td></tr>
...
<tr><td><code>int</code></td><td><code>15</code></td><td>1.2</td><td>memory</td><td><strong>Deprecated</strong> since 5.0, renamed to <code>org.glassfish.mojarra.numberOfViewSequencesInSession</code>, which inherits its value and is honored instead when both are set. It warns at startup when set. The old name said the opposite of what it sized.</td></tr>
<tr><th colspan="5" align="left"><br/><code>org.glassfish.mojarra.numberOfViewSequencesInSession</code></th></tr>
<tr><td><code>int</code></td><td><code>15</code></td><td>5.0</td><td>memory</td><td>Maximum amount of view sequences kept in the session when the state saving method is <code>server</code>. One view sequence covers an initially requested view and all of its postbacks, so in practice it bounds how many browser tabs retain their state. The session holds up to this value times <code>org.glassfish.mojarra.numberOfViewsPerViewSequence</code> states, so it directly sizes the session.</td></tr>
<tr><th colspan="5" align="left"><br/><code>org.glassfish.mojarra.numberOfViewsInSession</code></th></tr>
<tr><td><code>int</code></td><td><code>15</code></td><td>1.2</td><td>memory</td><td><strong>Deprecated</strong> since 5.0, renamed to <code>org.glassfish.mojarra.numberOfViewsPerViewSequence</code>, which inherits its value and is honored instead when both are set. It warns at startup when set. The old name said the opposite of what it sized.</td></tr>
<tr><th colspan="5" align="left"><br/><code>org.glassfish.mojarra.numberOfViewsPerViewSequence</code></th></tr>
<tr><td><code>int</code></td><td><code>15</code></td><td>5.0</td><td>memory</td><td>Maximum amount of views kept per view sequence when the state saving method is <code>server</code>, which in practice bounds how far back the browser back button still finds state within one tab. Together with <code>org.glassfish.mojarra.numberOfViewSequencesInSession</code> it bounds how much state a session holds.</td></tr>
