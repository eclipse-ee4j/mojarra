...
resolution or iteration. Their component counts are **not** equal — `flat-nopassthru` and `flat-passthru` carry 400,
`flat-readonly` and `flat-build` 800, `flat-text` 1600, `flat-allattrs` one block of every declared tag — because they
are calibrated by cost, not by size: a bare `h:outputText` renders far less than one carrying five attributes, and a
template-text node less again, so equal counts would park the cheap views near the noise floor where a pass-to-pass
swing of 10-25% says nothing. They all land around the ~2ms/request the rest of the suite sits at; tune a count
against the TOTAL column, not against the component count. The views are unrolled by hand rather than by a
`c:forEach`, precisely so that the build has no iteration in it — so changing a count means regenerating the
repeated block. Treat the round that lands new counts as a re-baseline: the WAR has to stay functionally unchanged
between rounds or cross-round comparisons stop meaning anything.
...
The `flat` family carries four further variants nothing else has, all GET:

- **passthru** — 400 `h:panelGroup layout="block" styleClass="cell"`, each with five `jakarta.faces.passthrough` `p:data-*` attributes: the only scenario exercising that feature, through the pass-through attribute handler at build time and `renderPassThruAttributes` at encode time. Two details are load-bearing and both were arrived at the hard way. The **namespace** is what makes them pass-through attributes at all: an attribute with no prefix carries namespace `""`, which `TagAttributesImpl.collectPassthrough` does not accept, so the same five names written without it miss every property descriptor, land in the plain attribute map and never reach the response. The **`styleClass`** (an `id` would serve equally) is what unconditionally forces the element to be emitted. Whether pass-through attributes on their own are enough to force it varies by implementation and by version, so a scenario relying on that renders nothing on some arm of the matrix and silently measures nothing — which is exactly how `h:outputText` failed here: Mojarra's `TextRenderer` treats pass-through attributes as a reason to emit a span, MyFaces does not, so that shape compared 400 spans against an empty response. Verify rendered output on both implementations before trusting any number from this scenario.
- **nopassthru** — the control for `passthru`: the same 400 `h:panelGroup layout="block" styleClass="cell"` at the same count, with the five `p:data-*` attributes removed and nothing else changed. Subtracting it from `passthru` is what isolates the pass-through path; both must keep the same multiplier, or the differential stops meaning anything. A control that also changed the component — the earlier one used `h:outputText` carrying five component properties — folds the whole renderer difference into the delta and isolates nothing.
- **allattrs** — one of every tag the HTML taglib declares, each carrying every attribute declared for it, generated from `faces.html.taglib.xml` so it follows the branch rather than a hand-written list. It is the coverage instrument: it says whether a change to the attribute machinery touches paths the narrow scenarios never reach (behavior-holder events, resource-backed attributes, converters, the select and table families). It is not a slope instrument — a block holds ~30 different components, so a regression in it does not localize on its own.
...
use slash form (`org/glassfish/mojarra/...`). To attribute a sample to a lifecycle phase, scan its
...
- Cancelling a bench mid-flight can leave an already-deployed app in `target/glassfish9/glassfish/domains/domain1/applications/`; the next run will then error with `Application with name perf-<version> is already registered`. Fix with `rm -rf target/glassfish9` followed by `mvn clean package -DskipTests`.
