...
- `view-unrolled-nested` — nested `c:forEach items` ∋ `c:forEach items` (output blocks, no inputs); *static* twin of `view-unrolled` but with a `c:forEach` nested inside a `c:forEach`, so the inner tag builds under an already-unrolled parent — exercising the nested `c:forEach` build/re-apply path that a single flat loop never reaches
