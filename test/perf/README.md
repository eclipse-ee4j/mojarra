...
- Cancelling a bench mid-flight can leave an already-deployed app in `target/glassfish7/glassfish/domains/domain1/applications/`; the next run will then error with `Application with name perf-4.0.19-SNAPSHOT is already registered`. Fix with `rm -rf target/glassfish7` followed by `mvn -pl test/perf -am package -DskipTests` (then reapply the JFR edit).
...
1. Build the layer in its own checkout (e.g. `mvn install` in a GlassFish module) so a `<version>-SNAPSHOT` artifact lands in `~/.m2`.
