...
type FacesGlobal = {
    faces: {
        ...
        getProjectStage(): FacesSpec.ProjectStage;
        getViewState(form: HTMLFormElement): string;
        getClientWindow(node?: HTMLElement | string): string | null;
        ajax: { response(req: XMLHttpRequest, ctx: AjaxContext): void };
    };
};
    ...
    const sendError = function sendError(
        ...
        if (description) {
            ...
            if (data.responseCode === 0) {
                data.description = "The Http Transport returned a 0 status code.  This is usually the result of mixing ajax and full requests.  " +
                    "This is usually undesired, for both performance and data integrity reasons.";
            }
            else {
    ...
    return {
        ...
        response: function response(request: XMLHttpRequest, context: AjaxContext) {
            ...
            if (responseType.nodeName !== "changes") {
                sendError(request, context, "malformedXML",
                    "Top level node must be one of: changes, redirect, error, received: " + responseType.nodeName + " instead.");
                return;
            }
            ...
            try {
                for (const change of Array.from(responseType.childNodes)) {
                    switch (change.nodeName) {
                        ...
                        default:
                            sendError(request, context, "malformedXML",
                                "Changes allowed are: update, delete, insert, attributes, eval, extension.  Received " + change.nodeName + " instead.");
                            return;
                    }
                }
            }
